...
def process_file(input_file, output_file):
    ...
    df["cnv_baseline_Id"] = df.apply(lambda row: cnv_baseline if ("INDIEGENE" in row["Test_Name"].upper() and row["Capturing_Kit"] in ["CE", "CEFu"] and row["Somatic_Germline"] in ["somatic", "germline"]) else (cnv_baseline_se8 if ("ABSOLUTE" in row["Test_Name"].upper() and row["Capturing_Kit"] == "SE8" and row["Somatic_Germline"] in ["somatic", "germline"]) else None), axis=1)
    df["baseline-noise-bed"] = df.apply(lambda row: baseline_noise if ("INDIEGENE" in row["Test_Name"].upper() and row["Capturing_Kit"] in ["CE", "CEFu"] and row["Somatic_Germline"] == "somatic") else (baseline_noise_se8 if ("ABSOLUTE" in row["Test_Name"].upper() and row["Capturing_Kit"] == "SE8" and row["Somatic_Germline"] == "somatic") else None), axis=1)
